...
const serviceLocationContent: Record<string, {
  ...
  'electric-sliding': {
    ...
    whyPoints: (city) => [
      `Sliding gate specialists in our ${city} network have direct experience with the ground conditions, gradients, and property types across this part of Hertfordshire.`,
      ...
      `FAAC, BFT, CAME, and Nice motor systems available, with established Hertfordshire parts networks for long-term maintenance.`,
    ],
  },
  'electric-swing': {
    intro: (city) => [
    ...
    whyPoints: (city) => [
      `Swing gate installers in our ${city} network understand the property types, plot sizes, and terrain across this part of Hertfordshire.`,
  ...
  'wooden-gates': {
    intro: (city) => [
      `Hardwood gates are the instinctive specification for a significant proportion of ${city} properties. The Green Belt, the Chilterns AONB, the conservation areas across Hertfordshire's historic towns, and the county's large stock of period farmhouses, barn conversions, and period farmhouses all create contexts where timber is not just preferred but is the only material that feels right. Where a planning officer or conservation adviser is involved, timber is also the specification that typically raises least resistance.`,
      `Fabrication is almost always bespoke rather than off-the-shelf for hardwood gates in ${city}. A skilled timber gate maker can match the proportions of the entrance, reference the architectural character of the house, or achieve a specific design detail, for a modest premium over a standard size. Iroko is the default choice for most projects: naturally durable, dimensionally stable, and consistent in quality. European oak is specified where the grain character is a design feature. Accoya, with its 50-year manufacturer guarantee, is the right specification for exposed or exposed Hertfordshire positions where low maintenance is a firm requirement.`,
    ],
    steps: (city) => [
      `Submit your enquiry and we connect you with hardwood gate specialists covering ${city} who have completed bespoke timber projects in this part of Hertfordshire.`,
    ...
    whyPoints: (city) => [
      `Hardwood gate specialists in our ${city} network understand Hertfordshire's AONB and conservation area planning context and can advise on the right specification.`,
  ...
  'metal-gates': {
    intro: (city) => [
      ...
      `For ${city} properties near the more exposed eastern Hertfordshire countryside, the corrosion challenge is more acute. Persistent dampness and greater weather exposure accelerate the breakdown of any poorly protected steel surface. Weather-resistant polyester powder coat over hot-dip galvanising is the recommended specification, and aluminium is worth considering as a zero-corrosion alternative that eliminates the concern entirely. Every installer in our ${city} network specifies hot-dip galvanising as standard and will advise on weather-resistant treatments where relevant.`,
    ],
    steps: (city) => [
  ...
  'automated-systems': {
    intro: (city) => [
      ...
      `The access control additions are where the investment often delivers the most practical value. A video intercom connected to a smartphone app means you can see and speak to anyone at the gate from anywhere and open or refuse entry remotely. A proximity reader opens the gate automatically as your vehicle approaches. For ${city} homeowners concerned about vehicle security, a closed automated gate with video recording adds a genuine deterrent layer against the relay theft methods that target premium driveways across Hertfordshire.`,
    ],
    steps: (city) => [
    ...
    whyPoints: (city) => [
      ...
      `FAAC, BFT, CAME, Nice, and Beninca motor systems available, with established parts networks across Hertfordshire.`,
