...
const serviceContent: Record<string, { intro: string[]; benefits: { title: string; desc: string }[]; candidateIntro: string; candidates: string[]; process: { title: string; desc: string }[] }> = {
  'electric-sliding': {
    intro: [
      "Hertfordshire driveways throw up conditions that favour sliding gates more often than you might expect from a county in the South East. The chalk ridge of the Chiltern Hills creates sharp elevation changes between road and entrance, particularly on properties between Wheathampstead and Hitchin where the scarp face drops steeply. Through the Hertfordshire countryside, between Radlett and Tring, clay soils on rolling terrain produce driveways that slope, curve, or both. In either situation, a sliding gate sidesteps the gradient problem by running horizontally along the boundary rather than sweeping through a vertical arc that the slope would obstruct.",
      "Two configurations account for nearly every Hertfordshire residential installation. Ground-track systems run on a steel channel set into a concrete foundation at surface level: the standard choice on flat or near-flat approaches. Cantilever systems suspend the gate from an elevated rail, removing the track entirely. The cantilever option comes into play where the gradient makes a level track unachievable without heavy earthwork, where a decorative block-paved or cobbled surface should not be cut, or where leaf fall from the mature broadleaf trees common on Hertfordshire rural properties would clog a ground channel within weeks of installation.",
      "Run-back space is the primary layout constraint. The leaf retracts to one side, requiring an unobstructed boundary at least equal to the gate width plus approximately 500mm for the motor housing. On many Hertfordshire properties this space exists naturally as a boundary wall, close-board fence, or established hedge line. Where it does not, a biparting configuration divides the gate into two leaves travelling in opposite directions, halving the run-back needed on each side.",
    ],
    benefits: [
      { title: 'Handles Hertfordshire Gradients Without Compromise', desc: 'The Chiltern Hills escarpment and the rolling rural Hertfordshire clay produce driveways that defeat standard swing gate geometry. A cantilever sliding gate clears the gradient entirely, and a ground-track system only requires a level base along the boundary rather than a flat driveway.' },
      { title: 'Stronger Engineering on Wide Openings', desc: 'Wide swing leaves concentrate enormous leverage at the hinge point. A sliding gate distributes load along the full length of the track. For Hertfordshire openings above 5 metres, the structural case for sliding over swing is straightforward.' },
      { title: 'Clean Footprint on Short Approaches', desc: 'When open, the leaf sits flat against the boundary with nothing projecting into the driveway. This matters on North Hertfordshire commuter belt properties and Welwyn Garden City suburban driveways where the approach from the road is short.' },
    ...
    candidateIntro: "A sliding gate is likely the right specification for your Hertfordshire property if:",
    candidates: [
  ...
  'electric-swing': {
    intro: [
      "Swing gates remain the most frequently installed gate type on detached Hertfordshire properties, and for sound reasons. A pair of leaves opening symmetrically as you arrive delivers a visual statement that a single sliding panel does not quite replicate. The format is mature: the motor technology, hinge engineering, and safety systems are all well understood, which means the installer population capable of executing the job reliably is larger than for any other gate type.",
      "The motor specification defines the character of the entrance as much as the gate material. Underground motors sit in a chamber below the post and are completely hidden when the gate is closed, producing an uncluttered entrance that reads as architecture rather than equipment. This is the standard specification across the West Hertfordshire premium market in St Albans, Harpenden, and Shenley. Ram-arm motors fix to the rear faces of gate and post, visible from inside the property. They are easier to service, cost less, and work well on retrofit projects where existing post foundations cannot accommodate excavation.",
      "Clearance is the suitability test. Each gate leaf must complete its full 90-degree arc without hitting vehicles, steps, raised borders, or anything else within the sweep path. Hertfordshire driveways that slope toward the house, common along the Chiltern Hills and through the Hertfordshire countrysideen hills, add a further consideration: the bottom edge of the leaf traces a descending path as it opens, and the motor and hinge specification must account for this. An experienced installer walks the full arc on site before committing to any specification.",
    ],
    benefits: [
      { title: 'Architectural Presence at the Entrance', desc: 'Paired gates opening from a central point create an entrance statement that a sliding leaf does not achieve. For Hertfordshire period properties, detached family homes, and any entrance designed to be noticed, swing gates are the natural format.' },
      ...
      { title: 'Concealed Motor Option for Premium Properties', desc: 'Underground motors are the expected specification in the St Albans and Harpenden belt. They keep the entrance visually clean and are available on both new installations and most retrofit projects where post foundations allow.' },
    ...
    candidateIntro: "Swing gates suit most Hertfordshire residential properties where the driveway geometry allows:",
    candidates: [
  ...
  'wooden-gates': {
    intro: [
      "Hertfordshire has one of the largest concentrations of properties in England where a hardwood gate is not merely preferred but is the only material that belongs. Period farmhouses and former agricultural buildings are found across the Hertfordshire countryside from Tring to Much Hadham. Timber-frame farmhouses and timber-frame houses line the lanes between Berkhamsted, Much Hadham, and Knebworth. Barn conversions occupy former agricultural buildings across the county. In each case, the material language of the property is natural, weathered, and organic. A powder-coated aluminium gate at the entrance to any of these buildings would be visually jarring. A well-specified hardwood gate continues the material conversation the property is already having.",
      "Three timber species account for the vast majority of Hertfordshire residential gate work. Iroko is the practical default: naturally durable to Use Class 3, dimensionally stable through the wet winters and dry summers of the South East, and consistent in quality. European oak brings deeper grain character and is specified where the timber itself is the design feature. It weathers to an attractive silver grey if left untreated, a finish that sits perfectly against the aged timber and local flint and brick found on rural Hertfordshire buildings. Accoya is the modern premium option: the acetylation process modifies the timber so fundamentally that the manufacturer certifies 50-year above-ground durability. For rural western Hertfordshire properties between Bishops Stortford and Hitchin, where weather exposure accelerates the breakdown of conventional softwoods, Accoya removes material longevity from the maintenance conversation entirely.",
      "Bespoke fabrication is the standard approach for timber gates in Hertfordshire, not an upgrade. A skilled gate maker can reference the proportions of your existing gateposts, the profile of the surrounding fencing, or a specific detail from the house. The cost premium over a catalogue size is modest, and the result is a gate designed for your entrance rather than selected from a range.",
    ],
    benefits: [
      { title: 'The Right Material for Hertfordshire Rural and Heritage Properties', desc: 'Conservation areas in Hertford, Ware, and the Hertfordshire countrysideen villages, both AONBs, and properties where the planning context favours traditional materials all benefit from hardwood. Timber is frequently the specification that planning officers expect.' },
      { title: 'Bespoke Design Without Premium Cost', desc: 'Joinery skills for hardwood gate fabrication are more widely available than specialist metalworking. A one-off design matched to your property adds modest cost over a standard size and is achievable through most Hertfordshire timber gate workshops.' },
      { title: 'Privacy and Noise Reduction', desc: 'A close-boarded hardwood gate provides complete visual screening and meaningful reduction in road noise. Relevant on Hertfordshire properties fronting A-roads and in villages with through-traffic.' },
    ...
    candidates: [
      "The property is an period farmhouse, barn conversion, farmhouse, period cottage, or any building where natural materials are architecturally correct",
      "The entrance falls within the Green Belt, the Chilterns AONB, or a conservation area where traditional materials carry planning weight",
  ...
  'metal-gates': {
    intro: [
      "Hertfordshire covers a broader metal gate market than most counties because the property stock is so varied. At one end, the St Albans and Harpenden premium market produces hand-forged wrought iron entrance treatments on brick piers with gilded finials and concealed motors. At the other, a laser-cut aluminium sliding gate in anthracite powder coat is the practical choice for a modern build in Watford, Potters Bar, or the Welwyn Garden City suburbs. Between these two points sits the bulk of Hertfordshire metal gate work: mild steel fabricated to a bespoke design, hot-dip galvanised, and powder-coated to the required colour.",
      "The surface treatment is the single most important specification decision. Hot-dip galvanising is the correct protection for any steel or iron gate that will live outdoors in Hertfordshire. The fabricated gate is immersed in molten zinc, bonding zinc to every surface including hollow section interiors, weld lines, and cut edges. The powder coat goes over the zinc. If the powder coat is chipped at any point in the gate's life, it exposes zinc rather than bare steel, and the zinc protects through a sacrificial mechanism. Exposed rural Hertfordshire properties face harsher weather conditions, and installers working across the open countryside should specify weather-resistant polyester powder coat or recommend aluminium where persistent dampness is a concern.",
      "Design flexibility in fabricated metal is substantial. Laser cutting produces intricate patterns, house names, and geometric motifs from flat plate with precision that hand fabrication cannot replicate. Traditional wrought iron profiles with curved heads, spear finials, and scrollwork are produced by specialist blacksmiths and widely specified on period properties in the Hertford and St Albans conservation areas. Most fabricators in our Hertfordshire network produce CAD drawings for approval, with 3D renders standard on larger projects.",
    ],
    benefits: [
    ...
    candidates: [
      ...
      "Your property sits in a Hertford or St Albans conservation area where heritage metalwork is expected",
    ...
    process: [
      ...
      { title: 'Fabrication and Surface Treatment', desc: 'The gate is fabricated, shot-blasted, hot-dip galvanised, and powder-coated in your RAL colour. Marine-grade powder coat for exposed Hertfordshire. Allow 3 to 6 weeks from drawing approval.' },
  ...
  'automated-systems': {
    ...
    benefits: [
      ...
      { title: 'Underground Motors on Most Hertfordshire Retrofits', desc: 'The preference for concealed motors in the West Hertfordshire market is well established. Most retrofit sites can accommodate underground installation with manageable additional work.' },
  ...
  'gate-repair': {
    intro: [
      "Gate failures across Hertfordshire follow predictable patterns. Motor gearboxes wear out from insufficient lubrication or from being undersized for the gate weight. Control boards fail when moisture reaches the electronics through poorly sealed housings. Photocell pairs drift out of alignment after a vehicle nudge or frost heave, causing false stops with no actual obstruction. Hinges that were never correctly adjusted at installation produce a gate leaf that drops until the motor stalls trying to push it through the cycle. On rural western Hertfordshire properties, add salt corrosion of control board contacts and relay switches to the list.",
      "The majority of these faults resolve in a single visit by an engineer carrying diagnostic tools and common spares for the brands most widely installed across Hertfordshire: FAAC, BFT, CAME, Nice, and Beninca. Brand matters to repair speed because parts availability depends on the manufacturer support network, and the established brands maintain active spare parts catalogues for at least a decade after a model leaves production. Unbranded or imported motors installed by less experienced firms create a different scenario: the motor fails and replacement parts either do not exist in the UK supply chain or must be sourced internationally at significant delay.",
    ...
    benefits: [
      ...
      { title: 'Van Stock for the Brands Hertfordshire Runs', desc: 'Engineers carry parts for FAAC, BFT, CAME, Nice, and Beninca. The majority of Hertfordshire callouts complete on the first visit because the right component is on the van.' },
